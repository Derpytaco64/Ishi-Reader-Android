...
internal class R2FXLLayout : FrameLayout {
    ...
    private fun internalScale(scale: Float, focusX: Float, focusY: Float) {
        ...
        matrixUpdated()
        // CLAUDE-PATCHED (upstream requestLayout() removed): scale is a canvas-time transform
        // (see dispatchDraw) that never changes this view's own measured size, and matrixUpdated()
        // above already recomputes drawRect/viewPortRect synchronously -- an Android layout pass
        // buys nothing here. Left in, requestLayout() fires on every single pinch/animated-zoom
        // frame (internalScale runs once per onScale callback and once per AnimatedZoomRunnable
        // tick); if any ancestor (this sits inside a ViewPager/FragmentContainerView stack) reflows
        // by even a rounding pixel in response, SimpleOnGlobalLayoutChangedListener.onGlobalLayout
        // below reacts to its own bounds "changing" by force-recentering the pan position to the
        // viewport's midpoint -- a plausible, unifying explanation for both remaining Ishi-Reader
        // comic-zoom bugs: pinch-zoom snapping back toward center mid-gesture, and double-tap zoom
        // (also driven by repeated internalScale calls, via AnimatedZoomRunnable) appearing to do
        // nothing because each animation frame gets immediately undone.
        invalidate()
    }
    ...
    private inner class SimpleOnGlobalLayoutChangedListener : ViewTreeObserver.OnGlobalLayoutListener {
        ...
        override fun onGlobalLayout() {
            ...
            // CLAUDE-PATCHED: don't fight an in-progress pinch or double-tap zoom animation --
            // re-clamping the pan mid-gesture is exactly what produced the pinch-zoom "snap to
            // center" bug (see internalScale's own comment). A real resize (rotation, window
            // change) still gets caught by AnimatedZoomRunnable.runValidation() at gesture end
