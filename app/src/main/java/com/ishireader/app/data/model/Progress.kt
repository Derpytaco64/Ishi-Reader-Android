 ...
 *  every percent figure in the app goes through (see [formatPercent] and [percentFromLocator]),
 *  including the exact percent ReaderActivity persists alongside a saved position (see
 *  PositionEntity.exactPercent), so a fraction always rounds to the same displayed number
 *  regardless of which caller formats it. */
